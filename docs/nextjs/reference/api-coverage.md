...
A topic becomes ✅ only after the handbook teaches the useful mental model, current API behavior, production implications, failure modes, and appropriate security/performance/testing/deployment/architecture trade-offs.
...
| Next.js 16.2 stable behavior | 🟠 | Core framework behavior, production operations and large-app architecture are complete; internals, migration, projects and interview system remain |
...
| `app` route tree / colocation | ✅ | Phases 01–02; architecture ownership Phase 18 |
...
| production checklist | ✅ | Performance 15; testing 16; deployment 17; architecture 18 |
...
| Area | Status | Notes |
| --- | --- | --- |
| pages / layouts / templates | ✅ | Phase 02 |
...
| parallel / intercepting routes | ✅ | Phase 02; rendering/tests 10/16 |
| `<Link>` / prefetching | ✅ | Phase 03; performance/testing 15–16 |
...
| hard vs soft navigation | ✅ | Phases 10 / 14–16; cross-zone hard navigation Phase 18 |
...
| --- | --- | --- |
| Server Components default | ✅ | Phase 04; architecture 18 |
| `'use client'` module graph | ✅ | Phase 04; security/performance 13/15; package boundaries 18 |
| Server → Client composition / serializable props | ✅ | Phase 04 |
| provider placement | ✅ | Phase 04; global-state governance 18 |
| `server-only` / `client-only` | ✅ | Phase 04; shared-package boundaries 18 |
...
| avoid own Route Handler hop from Server Component | ✅ | Phases 05 / 08; architecture 18 |
| parallel/sequential fetching, N+1, batching | ✅ | Phase 05; performance 15 |
| React `cache` / preload | ✅ | Phase 05 |
| external schema/status validation / timeouts / retries | ✅ | Phase 05 |
| secure DAL | ✅ | Phase 13; large-app ownership Phase 18 |
| DTO projection / field exposure | ✅ | Phase 13; architecture contract Phase 18 |
...
| Cache Components / `'use cache'` / `cacheLife` / `cacheTag` | ✅ | Phase 06 |
| `revalidateTag`, `updateTag`, `revalidatePath`, `refresh` | ✅ | Phases 06–07 |
| `connection()` | ✅ | Phase 06 |
| `'use cache: private'` | 🧪 | Experimental |
| partial prerendering through Cache Components | ✅ | Phases 06 / 10 |
...
| cache key/freshness/invalidation ownership | ✅ | Phase 18 |
| tenant/user cache identity | ✅ | Phases 06 / 13 / 17; architecture depth 18 |
...
| transactions / idempotency / outbox | ✅ | Phase 07; operations 17; command architecture 18 |
| Server Action tests | ✅ | Phase 16 |
| Server Action encryption/key consistency | ✅ | Phases 13 / 17 |
...
| request bodies / validation / files / streaming | ✅ | Phase 08 |
...
| reverse-proxy body/time/rate limits | ✅ | Phase 17 |
...
| Area | Status | Notes |
| --- | --- | --- |
| `proxy.ts` / request order / matchers | ✅ | Phase 09 |
...
| Area | Status | Notes |
| --- | --- | --- |
| RSC / HTML / hydration / Suspense / streaming | ✅ | Phase 10 |
| Cache Components shell / dynamic holes | ✅ | Phase 10 |
| streamed error/status caveats | ✅ | Phase 14 |
| end-to-end infrastructure streaming | ✅ | Phase 17 |
...
| Area / API | Status | Notes |
| --- | --- | --- |
| error boundaries / digests / `notFound()` / redirects | ✅ | Phase 14 |
| `after()` lifecycle | ✅ | Phase 14; durability distinction 17/18 |
...
| production build as release gate | ✅ | Phase 16; CI/CD 17 |
...
| Area / API | Status | Notes |
| --- | --- | --- |
| build/start/request lifecycle | ✅ | Phase 17 |
| `next build` / `next start` | ✅ | Phase 17 |
| output file tracing / `output: 'standalone'` / standalone server | ✅ | Phase 17 |
| Docker / read-only & ephemeral filesystem concerns | ✅ | Phase 17 |
| self-hosting / reverse proxy / forwarded-header trust | ✅ | Phase 17 |
| streaming through load balancer/proxy/CDN | ✅ | Phase 17 |
| graceful shutdown / pending `after()` | ✅ | Phase 17 |
| liveness/readiness / autoscaling / DB pools | ✅ | Phase 17 |
| build/runtime env and `NEXT_PUBLIC_` semantics | ✅ | Phase 17 |
| `cacheHandler` / `cacheHandlers` / distributed invalidation | ✅ | Phase 17 |
| `generateBuildId` / `deploymentId` / rolling skew | ✅ | Phase 17 |
...
| `serverExternalPackages` architecture context | ✅ | Performance 15; package boundary context 18 |
...
| cross-zone hard navigation / anchor semantics | ✅ | Phase 18 |
...
| custom Next.js server | 🟠 | Current guide says uncommon/ejection path; deep runtime/internals context Phase 19 |
...
| client-heavy SPA → server-first | 🟠 | Foundations through 18; deep 20 |
...
Phase 18 is complete for large-application architecture because it teaches:

- capability ownership, vertical slices, route composition roots, dependency direction, explicit module public APIs, architecture cycles and change/failure blast radius
- current App Router project-structure flexibility, safe colocation, private folders, route groups, multiple-root-layout trade-offs, feature-local UI and hybrid route/feature organisation
- DAL/query ownership, DTO exposure, domain policies, commands, transactions, idempotency, outbox/events, cache/invalidation ownership and server/client boundary design
- monorepos, workspace package boundaries, package exports, server/client entry points, design systems, stable `transpilePackages`, build/deployment tracing and automated dependency governance
- tenant identity and isolation across routing, sessions, DB, caches, custom domains, jobs, object storage, search, events, entitlements, rate limits, lifecycle and noisy-neighbour controls
- BFF/API architecture, direct Server Component reads, Server Actions as UI mutation adapters, provider adapters, synchronous dependency budgets, events/jobs/workflows, service extraction and contract compatibility
- feature flag/configuration/experiment/state governance, server-authoritative decisions, kill switches, rollout/schema compatibility, telemetry ownership and cross-cutting policy boundaries
- the official Multi-Zones model: path and asset ownership, hard navigation across zones, rewrite/proxy routing decisions, shared packages, session/Action compatibility, mixed-version rollout and team autonomy
- ADRs, architecture fitness functions, golden paths, data/cache/security/operational ownership, reference architecture and senior design-review checklists

Phase 19 now owns **Internals & Senior Mental Models**. Phase 20 owns upgrades/migration; projects and interview systems remain Phases 21–24.
