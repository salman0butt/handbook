...
description: "A migration is a production change to shared state. Design for lock duration, old/new application compatibility, backfill cost and rollback."
---
...
A migration is a production change to shared state. Design for lock duration, old/new application compatibility, backfill cost and rollback.
...
- **Forward-only** — forward-only and reversible migrations; connect the feature to correctness, execution cost, concurrency and operations.
- **Transactional** — transactional migrations; connect the feature to correctness, execution cost, concurrency and operations.
- **Expand-and-contract** — expand-and-contract; connect the feature to correctness, execution cost, concurrency and operations.
- **Zero-downtime** — zero-downtime changes; connect the feature to correctness, execution cost, concurrency and operations.
- **Safe** — safe columns and constraints; connect the feature to correctness, execution cost, concurrency and operations.
- **Create** — CREATE INDEX CONCURRENTLY; connect the feature to correctness, execution cost, concurrency and operations.
- **Type** — type changes and backfills; connect the feature to correctness, execution cost, concurrency and operations.
- **Renames** — renames and drops; connect the feature to correctness, execution cost, concurrency and operations.
- **Large-table** — large-table migration locks; connect the feature to correctness, execution cost, concurrency and operations.
- **Deployment** — deployment order, rollback and drift; connect the feature to correctness, execution cost, concurrency and operations.
- **Prisma,** — Prisma, Drizzle, Knex, TypeORM, Sequelize, Flyway and Liquibase; connect the feature to correctness, execution cost, concurrency and operations.
...
CHECK (fulfillment_state IN ('pending', 'packed', 'shipped'))
NOT VALID;

-- Backfill in bounded batches, deploy compatible code, then:
ALTER TABLE orders
VALIDATE CONSTRAINT orders_fulfillment_state_check;
```
