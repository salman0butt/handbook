...
description: "PostgreSQL gives each statement or transaction a visibility snapshot. Correctness depends on what the snapshot can see and which conflicts PostgreSQL detects."
---
...
PostgreSQL gives each statement or transaction a visibility snapshot. Correctness depends on what the snapshot can see and which conflicts PostgreSQL detects.
...
- **Acid** — ACID and transaction boundaries; connect the feature to correctness, execution cost, concurrency and operations.
- **Begin,** — BEGIN, COMMIT, ROLLBACK and savepoints; connect the feature to correctness, execution cost, concurrency and operations.
- **Autocommit** — autocommit; connect the feature to correctness, execution cost, concurrency and operations.
- **Tuple** — tuple versions and snapshots; connect the feature to correctness, execution cost, concurrency and operations.
- **Visibility** — visibility and MVCC; connect the feature to correctness, execution cost, concurrency and operations.
- **Read** — read committed, repeatable read and serializable; connect the feature to correctness, execution cost, concurrency and operations.
- **Non-repeatable** — non-repeatable reads, phantoms and write skew; connect the feature to correctness, execution cost, concurrency and operations.
- **Optimistic** — optimistic and pessimistic concurrency; connect the feature to correctness, execution cost, concurrency and operations.
- **Safe** — safe retries and long-running transaction risks; connect the feature to correctness, execution cost, concurrency and operations.
