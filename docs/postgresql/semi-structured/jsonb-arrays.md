---
title: "JSONB, Arrays and Semi-Structured Data"
description: "Use JSONB for variable attributes and document-shaped boundaries, while keeping stable identities, relationships and high-value invariants relational."
---

# JSONB, Arrays and Semi-Structured Data
...
Use JSONB for variable attributes and document-shaped boundaries, while keeping stable identities, relationships and high-value invariants relational.
...
- **Json** — JSON versus JSONB; connect the feature to correctness, execution cost, concurrency and operations.
- **Operators,** — operators, containment and JSON path; connect the feature to correctness, execution cost, concurrency and operations.
- **Nested** — nested querying and updates; connect the feature to correctness, execution cost, concurrency and operations.
- **Gin** — GIN indexes and operator classes; connect the feature to correctness, execution cost, concurrency and operations.
- **Generated** — generated relational columns from JSONB; connect the feature to correctness, execution cost, concurrency and operations.
- **Array** — array operators and indexes; connect the feature to correctness, execution cost, concurrency and operations.
- **Normalization** — normalization versus JSONB; connect the feature to correctness, execution cost, concurrency and operations.
- **Hybrid** — hybrid validation and performance trade-offs; connect the feature to correctness, execution cost, concurrency and operations.
...
SELECT product_id, attributes ->> 'brand' AS brand
FROM products
WHERE attributes @> '{"in_stock": true}'
  AND attributes @@ '$.dimensions.weight > 10';
```
