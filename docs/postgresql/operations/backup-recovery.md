...
description: "A backup is only trustworthy after a restore test. Design logical and physical recovery around explicit RPO and RTO targets."
---
...
A backup is only trustworthy after a restore test. Design logical and physical recovery around explicit RPO and RTO targets.
...
- **Pg_dump,** — pg_dump, pg_dumpall and pg_restore; connect the feature to correctness, execution cost, concurrency and operations.
- **Custom** — custom and directory formats; connect the feature to correctness, execution cost, concurrency and operations.
- **Schema-only,** — schema-only, data-only and selective restores; connect the feature to correctness, execution cost, concurrency and operations.
- **Parallel** — parallel restore; connect the feature to correctness, execution cost, concurrency and operations.
- **Base** — base backups and WAL archiving; connect the feature to correctness, execution cost, concurrency and operations.
- **Point-in-time** — point-in-time recovery; connect the feature to correctness, execution cost, concurrency and operations.
- **Rpo** — RPO and RTO; connect the feature to correctness, execution cost, concurrency and operations.
- **Backup** — backup verification, encryption and retention; connect the feature to correctness, execution cost, concurrency and operations.
- **Disaster-recovery** — disaster-recovery runbooks; connect the feature to correctness, execution cost, concurrency and operations.
...
pg_restore --dbname=app_restore_test --jobs=4 app.dump

# Verify schema, constraints, row counts and application smoke queries.
psql app_restore_test -c "select count(*) from orders;"
```
