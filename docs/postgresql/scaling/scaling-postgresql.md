...
description: "Scale vertically first when it is simplest, then separate connection, read, cache, partition and shard concerns instead of calling every growth problem 'scale'."
---
...
Scale vertically first when it is simplest, then separate connection, read, cache, partition and shard concerns instead of calling every growth problem 'scale'.
...
- **Vertical** — vertical scaling; connect the feature to correctness, execution cost, concurrency and operations.
- **Read** — read replicas; connect the feature to correctness, execution cost, concurrency and operations.
- **Pgbouncer** — PgBouncer pooling modes; connect the feature to correctness, execution cost, concurrency and operations.
- **Pooling** — pooling limitations; connect the feature to correctness, execution cost, concurrency and operations.
- **Application** — application caching; connect the feature to correctness, execution cost, concurrency and operations.
- **Partitioning** — partitioning; connect the feature to correctness, execution cost, concurrency and operations.
- **Citus** — Citus and sharding concepts; connect the feature to correctness, execution cost, concurrency and operations.
- **Tenant** — tenant sharding; connect the feature to correctness, execution cost, concurrency and operations.
- **Distributed** — distributed transaction trade-offs; connect the feature to correctness, execution cost, concurrency and operations.
- **Queues** — queues and CQRS; connect the feature to correctness, execution cost, concurrency and operations.
- **When** — when PostgreSQL is enough and when another system is needed; connect the feature to correctness, execution cost, concurrency and operations.
...
PgBouncer (transaction pool)
      |
primary PostgreSQL ---- streaming replica
      |
durable writes, WAL, backups and observability
```
