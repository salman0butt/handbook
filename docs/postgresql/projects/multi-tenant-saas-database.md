...
description: "Production blueprint for shared-schema tenant isolation with RLS."
---
...
Design shared-schema tenant isolation with RLS. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **tenants, users, memberships, projects and outbox events**. Stable facts remain relational; JSONB is limited to variable metadata. `tenant_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  TENANTS ||--o{ USERS : relates
  USERS ||--o{ MEMBERSHIPS : relates
  MEMBERSHIPS ||--o{ PROJECTS : relates
  PROJECTS ||--o{ OUTBOX_EVENTS : relates
```
...
Use tenant-scoped B-tree indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use SERIALIZABLE for cross-project quotas. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use tenant-safe backup/export and shard evolution. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
