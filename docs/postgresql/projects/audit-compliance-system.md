---
title: "Audit Logging and Compliance System"
description: "Production blueprint for tamper-evident operational history and access control."
---
...
Design tamper-evident operational history and access control. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **audit_events, actors, resources, retention_rules and exports**. Stable facts remain relational; JSONB is limited to variable metadata. `audit_event_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  AUDIT_EVENTS ||--o{ ACTORS : relates
  ACTORS ||--o{ RESOURCES : relates
  RESOURCES ||--o{ RETENTION_RULES : relates
  RETENTION_RULES ||--o{ EXPORTS : relates
```
...
Use time and resource indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use append-only writes and export snapshots. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use retention, legal holds and encrypted archives. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
