---
title: "Analytics and Reporting Database"
description: "Production blueprint for event ingestion and reproducible metrics."
---
...
Design event ingestion and reproducible metrics. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **events, dimensions, facts, metric_definitions and refresh_state**. Stable facts remain relational; JSONB is limited to variable metadata. `event_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  EVENTS ||--o{ DIMENSIONS : relates
  DIMENSIONS ||--o{ FACTS : relates
  FACTS ||--o{ METRIC_DEFINITIONS : relates
  METRIC_DEFINITIONS ||--o{ REFRESH_STATE : relates
```
...
Use BRIN and dimensional indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use batch transactions and refresh coordination. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use partition retention and materialized aggregates. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
