...
description: "Production blueprint for durable notification intent with transient wakeups."
---
...
Design durable notification intent with transient wakeups. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **notifications, deliveries, preferences and outbox_events**. Stable facts remain relational; JSONB is limited to variable metadata. `notification_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  NOTIFICATIONS ||--o{ DELIVERIES : relates
  DELIVERIES ||--o{ PREFERENCES : relates
  PREFERENCES ||--o{ OUTBOX_EVENTS : relates
```
...
Use recipient status indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use outbox and idempotent delivery claims. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use LISTEN/NOTIFY as hint, not durable queue. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
