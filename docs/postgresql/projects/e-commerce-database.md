...
description: "Production blueprint for orders, payments and inventory under concurrent checkout."
---
...
Design orders, payments and inventory under concurrent checkout. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **customers, products, inventory, orders, order_items and payments**. Stable facts remain relational; JSONB is limited to variable metadata. `order_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  CUSTOMERS ||--o{ PRODUCTS : relates
  PRODUCTS ||--o{ INVENTORY : relates
  INVENTORY ||--o{ ORDERS : relates
  ORDERS ||--o{ ORDER_ITEMS : relates
  ORDER_ITEMS ||--o{ PAYMENTS : relates
```
...
Use partial indexes for active orders. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use row locks for inventory reservation. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use catalog read scaling and order write integrity. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
