---
title: "Job Queue with SKIP LOCKED"
description: "Production blueprint for a concurrent durable worker queue."
---
...
Design a concurrent durable worker queue. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **jobs, job_attempts and dead_letters**. Stable facts remain relational; JSONB is limited to variable metadata. `job_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  JOBS ||--o{ JOB_ATTEMPTS : relates
  JOB_ATTEMPTS ||--o{ DEAD_LETTERS : relates
```
...
Use partial ready-job indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use FOR UPDATE SKIP LOCKED with leases. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use backpressure, retries and poison-message handling. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
