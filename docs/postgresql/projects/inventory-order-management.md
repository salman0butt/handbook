...
description: "Production blueprint for stock, reservations, orders and fulfillment."
---
...
Design stock, reservations, orders and fulfillment. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **warehouses, stock, reservations, orders and shipments**. Stable facts remain relational; JSONB is limited to variable metadata. `reservation_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  WAREHOUSES ||--o{ STOCK : relates
  STOCK ||--o{ RESERVATIONS : relates
  RESERVATIONS ||--o{ ORDERS : relates
  ORDERS ||--o{ SHIPMENTS : relates
```
...
Use SKU and reservation-expiry indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use row locking with deterministic order. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use reconciliation, expiry and warehouse partitioning. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
