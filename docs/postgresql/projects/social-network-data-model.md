---
title: "Social Network Data Model"
description: "Production blueprint for follows, posts, feeds and moderation."
---
...
Design follows, posts, feeds and moderation. The database must keep invariants correct during retries, concurrent requests, migrations, partial failures and operator intervention. This blueprint targets PostgreSQL 18.4.
...
Core relations: **users, follows, posts, reactions and moderation_actions**. Stable facts remain relational; JSONB is limited to variable metadata. `user_id` is the main durable identity. Every tenant or ownership boundary is present in keys, foreign keys, queries and policies rather than inferred from application routing.
...
```mermaid
erDiagram
  USERS ||--o{ FOLLOWS : relates
  FOLLOWS ||--o{ POSTS : relates
  POSTS ||--o{ REACTIONS : relates
  REACTIONS ||--o{ MODERATION_ACTIONS : relates
```
...
Use feed and relationship indexes. Add indexes for foreign-key lookup and measured access paths, not every column. Verify column order, predicates and operator classes with representative plans. Track unused, duplicate and bloated indexes because each one increases write amplification and WAL.
...
Use idempotent writes and counter reconciliation. Lock rows in a deterministic order. Set bounded `lock_timeout` and `statement_timeout`, and retry complete transactions only for classified transient errors such as serialization failures or deadlocks. Keep network calls outside open transactions.
...
Use partitioned activity and read models. Scale connection handling, reads, writes, history and analytics independently. Prefer vertical scaling and workload correction before adding distributed coordination. Document consistency changes introduced by replicas, caches or shards.
