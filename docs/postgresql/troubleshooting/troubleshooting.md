...
description: "Troubleshoot with falsifiable hypotheses: reproduce, classify the wait or error, gather evidence, change one variable and verify recovery."
---
...
Troubleshoot with falsifiable hypotheses: reproduce, classify the wait or error, gather evidence, change one variable and verify recovery.
...
- **Connection** — connection and authentication failures; connect the feature to correctness, execution cost, concurrency and operations.
- **Role,** — role, database and permission errors; connect the feature to correctness, execution cost, concurrency and operations.
- **Too** — too many connections and pool exhaustion; connect the feature to correctness, execution cost, concurrency and operations.
- **Timeouts,** — timeouts, deadlocks and blocked queries; connect the feature to correctness, execution cost, concurrency and operations.
- **Slow** — slow queries and missing or unused indexes; connect the feature to correctness, execution cost, concurrency and operations.
- **Cpu,** — CPU, memory and disk pressure; connect the feature to correctness, execution cost, concurrency and operations.
- **Table/index** — table/index bloat and autovacuum lag; connect the feature to correctness, execution cost, concurrency and operations.
- **Replica** — replica lag, slot growth and WAL growth; connect the feature to correctness, execution cost, concurrency and operations.
- **Failed** — failed and lock-heavy migrations; connect the feature to correctness, execution cost, concurrency and operations.
- **Backup,** — backup, restore, collation, extension and time-zone problems; connect the feature to correctness, execution cost, concurrency and operations.
- **Null,** — NULL, duplicate-row and isolation bugs; connect the feature to correctness, execution cost, concurrency and operations.
