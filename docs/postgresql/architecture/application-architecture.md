---
title: "PostgreSQL Application Architecture"
description: "Put transaction boundaries around business invariants, not arbitrary repository calls. Make delivery, idempotency and concurrency explicit."
---
...
Put transaction boundaries around business invariants, not arbitrary repository calls. Make delivery, idempotency and concurrency explicit.
...
- **Repository** — repository layer and unit of work; connect the feature to correctness, execution cost, concurrency and operations.
- **Transaction** — transaction boundaries; connect the feature to correctness, execution cost, concurrency and operations.
- **Outbox** — outbox and inbox patterns; connect the feature to correctness, execution cost, concurrency and operations.
- **Idempotency** — idempotency keys; connect the feature to correctness, execution cost, concurrency and operations.
- **Sagas** — sagas and job queues; connect the feature to correctness, execution cost, concurrency and operations.
- **Event-driven** — event-driven systems and audit logs; connect the feature to correctness, execution cost, concurrency and operations.
- **Optimistic** — optimistic and pessimistic locking; connect the feature to correctness, execution cost, concurrency and operations.
- **Multi-tenant** — multi-tenant SaaS; connect the feature to correctness, execution cost, concurrency and operations.
- **Financial** — financial ledgers and append-only models; connect the feature to correctness, execution cost, concurrency and operations.
- **Temporal** — temporal data, CQRS and read models; connect the feature to correctness, execution cost, concurrency and operations.
- **Sessions,** — sessions, rate limits, feature flags and notifications; connect the feature to correctness, execution cost, concurrency and operations.
...
VALUES ($4, $1, 'order.created', $5::jsonb);

COMMIT;
-- A separate idempotent publisher locks and delivers outbox rows.
```
