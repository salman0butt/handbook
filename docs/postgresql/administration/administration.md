...
description: "Administration manages cluster-wide identity, configuration, lifecycle and maintenance while respecting which changes require reload, restart or migration."
---
...
Administration manages cluster-wide identity, configuration, lifecycle and maintenance while respecting which changes require reload, restart or migration.
...
- **Databases,** — databases, schemas, roles and ownership; connect the feature to correctness, execution cost, concurrency and operations.
- **Configuration** — configuration reload versus restart; connect the feature to correctness, execution cost, concurrency and operations.
- **Extensions** — extensions; connect the feature to correctness, execution cost, concurrency and operations.
- **Collation,** — collation, locale and encoding; connect the feature to correctness, execution cost, concurrency and operations.
- **Maintenance** — maintenance and tablespaces; connect the feature to correctness, execution cost, concurrency and operations.
- **Connection** — connection and session management; connect the feature to correctness, execution cost, concurrency and operations.
- **Database** — database cloning and templates; connect the feature to correctness, execution cost, concurrency and operations.
- **Statistics** — statistics maintenance; connect the feature to correctness, execution cost, concurrency and operations.
- **Minor** — minor and major upgrades; connect the feature to correctness, execution cost, concurrency and operations.
- **Pg_upgrade** — pg_upgrade and logical upgrade strategies; connect the feature to correctness, execution cost, concurrency and operations.
- **Extension** — extension upgrades and maintenance windows; connect the feature to correctness, execution cost, concurrency and operations.

## Practical example

```bash
psql -X -d postgres -c "select current_setting('server_version');"
psql -X -d postgres -c "select pg_reload_conf();"
psql -X -d postgres -c "select name, setting, pending_restart
