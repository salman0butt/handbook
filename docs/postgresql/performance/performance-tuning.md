...
description: "Measure the workload, identify the dominant wait or work, change one hypothesis, and compare. Tuning without a baseline is guesswork."
---
...
Measure the workload, identify the dominant wait or work, change one hypothesis, and compare. Tuning without a baseline is guesswork.
...
- **Slow-query** — slow-query logging and pg_stat_statements; connect the feature to correctness, execution cost, concurrency and operations.
- **Pg_stat_activity** — pg_stat_activity and wait events; connect the feature to correctness, execution cost, concurrency and operations.
- **Table** — table and index statistics; connect the feature to correctness, execution cost, concurrency and operations.
- **Shared_buffers,** — shared_buffers, work_mem and effective_cache_size; connect the feature to correctness, execution cost, concurrency and operations.
- **Connection** — connection overhead and pooling; connect the feature to correctness, execution cost, concurrency and operations.
- **Batching,** — batching, COPY and bulk loading; connect the feature to correctness, execution cost, concurrency and operations.
- **Offset** — offset versus keyset pagination; connect the feature to correctness, execution cost, concurrency and operations.
- **N+1** — N+1 queries; connect the feature to correctness, execution cost, concurrency and operations.
- **Wal,** — WAL, checkpoint, vacuum and bloat pressure; connect the feature to correctness, execution cost, concurrency and operations.
- **Hardware,** — hardware, network latency and production checklists; connect the feature to correctness, execution cost, concurrency and operations.
