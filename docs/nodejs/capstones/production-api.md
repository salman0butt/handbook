---
title: Capstone 2 — Production Express API
...
# Capstone 2 — Production Express API

## Requirements

Build a production order-management API with Express, TypeScript, PostgreSQL, Redis, authentication, object authorization, idempotency, observability, Docker and CI.
...
```mermaid
flowchart LR
  C1["Reverse proxy and Express"]
  C2["Order module"]
  C3["PostgreSQL transactions"]
  C4["Redis cache and limits"]
  C5["Outbox and queue"]
  C6["Observability platform"]
  C1 --> C2
  C2 --> C3
  C3 --> C4
  C4 --> C5
...
- `Reverse proxy and Express`
- `Order module`
- `PostgreSQL transactions`
- `Redis cache and limits`
- `Outbox and queue`
...
Model users, tenants, products, orders, order_items, payments, idempotency_keys and outbox_events. Use unique constraints for idempotency and explicit order state transitions.
...
Expose versioned REST resources, cursor pagination, filters, Problem Details, OpenAPI, idempotent order creation and signed outbound webhooks.
...
Use OIDC login or secure sessions for users and scoped service credentials for internal calls. Rotate refresh/session credentials and implement recovery controls.
...
Cache product/catalog reads, not order authorization decisions. Add request coalescing and tenant-aware keys.
...
Process payment confirmation, email, inventory reservation and webhook delivery asynchronously with outbox-backed jobs.
...
- payment provider timeout after charge
- duplicate POST retry across replicas
- Redis outage
- database deadlock
- stale inventory
- webhook consumer returns 500
- SIGTERM during checkout
...
Scale Express replicas behind a load balancer, size DB pools globally, partition jobs by resource, add read models and measure p99 before decomposing.

Scale only after measuring the first constrained resource. Include database and provider capacity, not only Node replicas.

## Extensions

- Add fraud-review workflow
- Add canary releases and synthetic checks
- Extract payment adapter as a service only when ownership requires it
