---
title: Capstone 7 — Realtime Chat and Collaboration System
...
# Capstone 7 — Realtime Chat and Collaboration System

## Requirements

Build rooms, presence, typing, durable messages, edits, reactions, read state and collaborative updates over WebSockets or SSE plus HTTP.
...
```mermaid
flowchart LR
  C1["HTTP auth and history API"]
  C2["Realtime gateways"]
  C3["Presence and room service"]
  C4["Durable message store"]
  C5["Pub sub and replay"]
  C6["Moderation and telemetry"]
  C1 --> C2
  C2 --> C3
  C3 --> C4
  C4 --> C5
...
- `HTTP auth and history API`
- `Realtime gateways`
- `Presence and room service`
...
Store durable messages, room membership, sequence IDs, edits and read cursors. Keep ephemeral presence separate from durable history.
...
Use HTTP for history and commands that need idempotent responses; use realtime transport for low-latency events with message IDs and replay cursors.
...
Authenticate connection establishment and authorize every room join, message, edit and moderation action.
...
Cache room metadata and recent history with membership-aware keys; presence may use expiring Redis data.
...
Run notification fan-out, moderation, media scanning, history compaction and offline delivery in queues.
...
- slow consumer buffers grow
- duplicate send after timeout
- Redis Pub/Sub gap
- room membership changes mid-connection
...
Distribute connections, use a broker or stream for fan-out/replay, partition rooms, control hot-room fan-out and apply per-user/tenant connection quotas.

Scale only after measuring the first constrained resource. Include database and provider capacity, not only Node replicas.

## Extensions

- Add CRDT document collaboration
- Add end-to-end encryption design
- Add regional edge gateways
