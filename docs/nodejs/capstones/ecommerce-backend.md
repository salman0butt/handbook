---
title: Capstone 5 — E-Commerce Backend
...
# Capstone 5 — E-Commerce Backend

## Requirements

Design catalog, inventory, cart, checkout, payment, fulfillment, refunds and customer-notification workflows with strong idempotency and failure recovery.
...
flowchart LR
  C1["Storefront API"]
  C2["Catalog and pricing"]
  C3["Cart and checkout"]
  C4["Inventory and order database"]
  C5["Payment and workflow jobs"]
  C6["Search and observability"]
  C1 --> C2
  C2 --> C3
  C3 --> C4
  C4 --> C5
...
Separate catalog, pricing, inventory reservations, orders, payments and fulfillment state. Use immutable money/currency fields, reservation expiry and an outbox.
...
Expose catalog search, cart commands, quote/checkout, order status, returns and admin operations. Prices and entitlements are server-derived.
...
Use customer sessions/OIDC and staff roles with object-level checks; provider webhooks authenticate independently.
...
Cache catalog and search results with versioned keys; never use stale cache as authoritative inventory or final price.
...
Run payment confirmation, reservation expiry, fulfillment, refund, email and webhook workflows with dedupe and compensation.
...
Partition catalog reads, isolate checkout writes, use queues for fan-out, add regional read replicas, and preserve a single invariant owner for inventory/order transitions.

Scale only after measuring the first constrained resource. Include database and provider capacity, not only Node replicas.

## Extensions

- Add marketplace sellers
- Add tax provider abstraction
- Add event-driven analytics projection
