---
title: Capstone — Vanilla JavaScript Application Platform
...
id: 11-capstone-vanilla-javascript-application-platform
---

# Capstone — Vanilla JavaScript Application Platform

**Goal.** Create a production-style vanilla JavaScript platform with ESM, routing, API communication, validation, cancellation, DOM rendering, storage, logging, tests, workers, offline/degraded states, accessibility, security, performance measurement, and deployment.
...
### Architecture
Use ports/adapters, explicit state ownership, validated trust boundaries, cancellation, observability, and documented migration seams.

```text
browser host → app shell → use cases → domain → ports → infrastructure
```

**Suggested module structure:** `src/app/, src/domain/, src/features/, src/platform/http/, src/platform/storage/, src/platform/worker/, src/platform/telemetry/, test/`
