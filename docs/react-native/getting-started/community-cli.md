---
title: Community CLI Application Workflow
description: Create, run, select devices, choose variants, clean the correct layer and troubleshoot the first native build.
---

# Community CLI Application Workflow

Create, run, select devices, choose variants, clean the correct layer and troubleshoot the first native build. This page is a canonical production guide for the React Native 0.86 Community CLI baseline. It separates React Native framework behavior, third-party library behavior and Android/iOS platform ownership so version-specific configuration remains visible.

```mermaid
flowchart LR
  A["Source and configuration"] --> B["Deterministic toolchain"]
  B --> C["Build or diagnostic artifact"]
  C --> D["Measured decision"]
...
export function assertProduction(identity: BuildIdentity): void {
  if (identity.environment !== 'production') {
    throw new Error('Expected production build identity')
  }
}
```
