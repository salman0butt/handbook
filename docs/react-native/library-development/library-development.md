---
title: React Native Library Development
description: Package JavaScript and native libraries with TypeScript, Codegen, TurboModules, Fabric, examples, tests and compatibility policy.
---

# React Native Library Development

Package JavaScript and native libraries with TypeScript, Codegen, TurboModules, Fabric, examples, tests and compatibility policy. This page is a canonical production guide for the React Native 0.86 Community CLI baseline. It separates React Native framework behavior, third-party library behavior and Android/iOS platform ownership so version-specific configuration remains visible.

```mermaid
flowchart LR
  A["TypeScript contract"] --> B["Autolinking and Codegen"]
  B --> C["Android and iOS configuration"]
  C --> D["Physical device behavior"]
...
export async function requireCapability<T>(capability: NativeCapability<T>): Promise<T> {
  if (!(await capability.isAvailable())) throw new Error('CAPABILITY_UNAVAILABLE')
