...
description: Production blueprint for notes and cross-device synchronization using React Native Community CLI.
---
...
Build a maintainable notes and cross-device synchronization product with typed boundaries, accessible interaction, observable failures, secure configuration and deliberate Android/iOS ownership. Demonstrate end-to-end responsibility from product requirements through deployment and incident response.

## Requirements

- React Native 0.86 Community CLI project with strict TypeScript.
- Notes list, editor, conflicts, search and settings.
...
1. A user can complete the primary notes and cross-device synchronization journey with clear feedback.
...
flowchart TD
  ...
  APP --> DOMAIN["notes and cross-device synchronization policy"]
  APP --> API["API repository"]
  APP --> STORE["Local storage and sync queue"]
  APP --> NATIVE["files, share sheet and background task constraints"]
...
src/
  app/
  navigation/
  features/offline-first-notes-application/
    application/
    domain/
    data/
    screens/
...
The required screens are: Notes list, editor, conflicts, search and settings. Each screen documents entry routes, ownership, loading/empty/error states, accessibility focus, analytics events and destructive-action confirmation.
...
Core entities are Note, Revision, Attachment and SyncCursor. Define stable IDs, timestamps, schema versions and ownership. Every queued mutation has an idempotency key, attempt count, next retry time and terminal failure reason.
...
SQLite source of truth with operation log. Reads expose freshness. Writes are idempotent and queued with capped backoff. Conflict policy is domain-specific and visible to the user where automatic resolution could destroy intent.
...
encrypted sensitive fields and safe attachment handling. Keep secrets out of bundles, validate deep links, redact observability data, minimize permissions and document dependency/native SDK risk. Client-side checks never replace server authorization.
...
Practice conflicting edits, corrupt database and exhausted storage. For each incident, record detection, user impact, containment, evidence, root cause, repair, rollout and prevention. Avoid random cache clearing before preserving the first useful logs.
