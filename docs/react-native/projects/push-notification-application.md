---
title: Push Notification Application
description: Production blueprint for notification preferences and routed delivery using React Native Community CLI.
---
...
Build a maintainable notification preferences and routed delivery product with typed boundaries, accessible interaction, observable failures, secure configuration and deliberate Android/iOS ownership. Demonstrate end-to-end responsibility from product requirements through deployment and incident response.

## Requirements

- React Native 0.86 Community CLI project with strict TypeScript.
- Permission education, preferences, inbox and notification details.
...
1. A user can complete the primary notification preferences and routed delivery journey with clear feedback.
...
flowchart TD
  ...
  APP --> DOMAIN["notification preferences and routed delivery policy"]
  APP --> API["API repository"]
  APP --> STORE["Local storage and sync queue"]
  APP --> NATIVE["APNs, FCM, channels, categories and deep links"]
...
src/
  app/
  navigation/
  features/push-notification-application/
    application/
    domain/
    data/
    screens/
...
The required screens are: Permission education, preferences, inbox and notification details. Each screen documents entry routes, ownership, loading/empty/error states, accessibility focus, analytics events and destructive-action confirmation.
...
Core entities are DeviceToken, Subscription, Notification and DeliveryReceipt. Define stable IDs, timestamps, schema versions and ownership. Every queued mutation has an idempotency key, attempt count, next retry time and terminal failure reason.
...
local inbox and deferred route intent. Reads expose freshness. Writes are idempotent and queued with capped backoff. Conflict policy is domain-specific and visible to the user where automatic resolution could destroy intent.
...
no sensitive lock-screen content by default. Keep secrets out of bundles, validate deep links, redact observability data, minimize permissions and document dependency/native SDK risk. Client-side checks never replace server authorization.
...
Practice token refresh, revoked permission and duplicate delivery. For each incident, record detection, user impact, containment, evidence, root cause, repair, rollout and prevention. Avoid random cache clearing before preserving the first useful logs.
