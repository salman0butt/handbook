...
description: Production blueprint for catalog, cart and checkout using React Native Community CLI.
---
...
Build a maintainable catalog, cart and checkout product with typed boundaries, accessible interaction, observable failures, secure configuration and deliberate Android/iOS ownership. Demonstrate end-to-end responsibility from product requirements through deployment and incident response.

## Requirements

- React Native 0.86 Community CLI project with strict TypeScript.
- Catalog, search, product, cart, checkout and orders.
...
1. A user can complete the primary catalog, cart and checkout journey with clear feedback.
...
flowchart TD
  ...
  APP --> DOMAIN["catalog, cart and checkout policy"]
  APP --> API["API repository"]
  APP --> STORE["Local storage and sync queue"]
  APP --> NATIVE["deep links, share sheet and wallet/payment SDK boundary"]
...
src/
  app/
  navigation/
  features/ecommerce-mobile-application/
    application/
    domain/
    data/
    screens/
...
The required screens are: Catalog, search, product, cart, checkout and orders. Each screen documents entry routes, ownership, loading/empty/error states, accessibility focus, analytics events and destructive-action confirmation.
...
Core entities are Product, Cart, Order, Address and PaymentIntent. Define stable IDs, timestamps, schema versions and ownership. Every queued mutation has an idempotency key, attempt count, next retry time and terminal failure reason.
...
cached catalog and durable cart. Reads expose freshness. Writes are idempotent and queued with capped backoff. Conflict policy is domain-specific and visible to the user where automatic resolution could destroy intent.
...
server-authoritative pricing and payment confirmation. Keep secrets out of bundles, validate deep links, redact observability data, minimize permissions and document dependency/native SDK risk. Client-side checks never replace server authorization.
...
Practice stale inventory, duplicate checkout and interrupted payment. For each incident, record detection, user impact, containment, evidence, root cause, repair, rollout and prevention. Avoid random cache clearing before preserving the first useful logs.
