...
description: Production blueprint for conversations and messaging using React Native Community CLI.
---
...
Build a maintainable conversations and messaging product with typed boundaries, accessible interaction, observable failures, secure configuration and deliberate Android/iOS ownership. Demonstrate end-to-end responsibility from product requirements through deployment and incident response.

## Requirements

- React Native 0.86 Community CLI project with strict TypeScript.
- Conversation list, thread, composer, media preview and member details.
...
1. A user can complete the primary conversations and messaging journey with clear feedback.
...
flowchart TD
  ...
  APP --> DOMAIN["conversations and messaging policy"]
  APP --> API["API repository"]
  APP --> STORE["Local storage and sync queue"]
  APP --> NATIVE["push notifications, camera, media and keyboard"]
...
src/
  app/
  navigation/
  features/realtime-chat-application/
    application/
    domain/
    data/
    screens/
...
The required screens are: Conversation list, thread, composer, media preview and member details. Each screen documents entry routes, ownership, loading/empty/error states, accessibility focus, analytics events and destructive-action confirmation.
...
Core entities are Conversation, Message, Receipt, Attachment and Member. Define stable IDs, timestamps, schema versions and ownership. Every queued mutation has an idempotency key, attempt count, next retry time and terminal failure reason.
...
outbox with client-generated IDs and deduplication. Reads expose freshness. Writes are idempotent and queued with capped backoff. Conflict policy is domain-specific and visible to the user where automatic resolution could destroy intent.
...
authorization, upload validation and sensitive notification policy. Keep secrets out of bundles, validate deep links, redact observability data, minimize permissions and document dependency/native SDK risk. Client-side checks never replace server authorization.
...
Practice reconnect gaps, duplicate messages and out-of-order receipts. For each incident, record detection, user impact, containment, evidence, root cause, repair, rollout and prevention. Avoid random cache clearing before preserving the first useful logs.
