---
title: Styling, Flexbox and Responsive Layout
description: Build density-aware, safe-area-aware, platform-adaptive layouts with design tokens and accessible typography.
---

# Styling, Flexbox and Responsive Layout

Build density-aware, safe-area-aware, platform-adaptive layouts with design tokens and accessible typography. This page is a canonical production guide for the React Native 0.86 Community CLI baseline. It separates React Native framework behavior, third-party library behavior and Android/iOS platform ownership so version-specific configuration remains visible.

```mermaid
flowchart LR
  A["Input or state"] --> B["Pure React render"]
  B --> C["Fabric and Yoga"]
  C --> D["Accessible native UI"]
...
export function renderLabel(state: Loadable<unknown>): string {
  switch (state.status) {
