---
title: Application Lifecycle and Background Work
description: Handle AppState, process death, suspension, background tasks, idempotency, battery restrictions and restoration.
---

# Application Lifecycle and Background Work

Handle AppState, process death, suspension, background tasks, idempotency, battery restrictions and restoration. This page is a canonical production guide for the React Native 0.86 Community CLI baseline. It separates React Native framework behavior, third-party library behavior and Android/iOS platform ownership so version-specific configuration remains visible.

```mermaid
flowchart LR
  A["User intent"] --> B["Cancelable operation"]
  B --> C["Typed outcome"]
  C --> D["Recovery and telemetry"]
